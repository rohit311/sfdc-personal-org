({  
        ...
        this.executeApex(component, 'getPricingDashboardDetails', {
            ...
            if(!error && result){
                    ...
                    /* for(insurance__c obj : insuranceList)
                {
                    ...
                    {
                        console.log('ecs0' + (repaymentModeDtlList[0].UMRN__c )); 
                    ...
                    if(!$A.util.isEmpty(disbursalList) && !($A.util.isEmpty(repaymentModeDtlList))){
                        console.log('Repay_Disb_Diff__c'+disbursalList[0].Repay_Disb_Diff__c);
                       	if(disbursalList[0].Repay_Disb_Diff__c){
                        	component.set("v.isBanking",false);
                        }   
                        else{
                        	if((disbursalList[0].IFSC_Code__c != repaymentModeDtlList[0].IFSC_Code__c) || (disbursalList[0].Bank_Account__c != repaymentModeDtlList[0].A_C_NO__c))
                                component.set("v.isBanking",false);
                            else
                    ...
                    if(!($A.util.isEmpty(verificationList)))
                    {
                        for(var i = 0 ; i< verificationList.length ;i++)
                        {
                            ...
                            {
                                if(verificationList[i].Credit_Status__c == 'Complete' || verificationList[i].Credit_Status__c == 'Not Required')
                                    component.set("v.isCurrentAddressVerified",true);
                    ...
                    if(!$A.util.isEmpty(loanApplication))
                    {
                    ...
                    if(!($A.util.isEmpty(disbursalList)))
                    {
                        ...
                        {	
                            //console.log("disbursal "+disbursalList[i].IMPS_Flag__c);
                    ...
                    if(!($A.util.isEmpty(verificationList)))
                    {
                        ...
                        for(var i=0;i<verificationList.length ; i++)
                        {
                            ...
                            {	
                                console.log('is residense or office');
                                if(verificationList[i].Geo_Tagging__c == false)
                                {
                                    component.set("v.isGeoTagging",true);
                                    component.set("v.isGeoTaggingToBranchOps",true);
                                    console.log('geo_tagging not applicable');
                                }
                                else if(verificationList[i].Geo_Tagging__c == true && verificationList[i].Image_Latitude_Longitude_Details__c  != null){
                                    component.set("v.isGeoTaggingToBranchOps",true);    
                                    //component.set("v.isGeoTagging",true);
                                } //This condition used to send to branch ops
                                    else{
    ...
    colorDashboard : function (component,event){
        ...
        if(component.get("v.isGeoTaggingToBranchOps")==true)
        {
            iconMap.push({
                name: 'Geo Tagging',
                value: 'action:approval'
    ...
    sendToBranchOps : function (component,event){
        ...
        if( deDupe == false || insurance == false || discrepancy == false || 
           bankVerification == false || officeVerification == false || residenceVerification == false || permanentVerification == false || geoTagging == false || geoTaggingBranchOps == false )
        {
        ...
        {
            this.uploadVerification(component,event);    
            console.log(component.get("v.isSendToFinnOne"));
            ...
            else if((isBanking == true || component.get("v.isUploadBanking")) && (isAppForm == true || component.get("v.isUploadAppForm") == true) && (customerConsent == true || component.get("v.isUploadCustomerConsent") == true)&& (eKyc == true||component.get("v.isUploadeKyc") == true) && (eMandate == true||component.get("v.isUploadeMandate") == true) && (IMPS == true||component.get("v.isUploadIMPS") == true) && (SPDC == true || component.get("v.isUploadSPDC") == true) 
